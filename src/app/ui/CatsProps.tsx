...
export default function ProdCodeItems({
  catsProds,
  lang,
}: {
  catsProds: any;
  ...
  const filteredItems =
    search !== "" || !search
      ? catsProds.categories?.products.filter((item: any) =>
          item.name.toLowerCase().includes(search.toLowerCase())
        )
      : catsProds.categories?.products;
  ...
  return (
    <>
      <div className="w-full lg:min-w-5xl h-3/4 overflow-auto max-w-7xl rounded-2xl flex flex-wrap gap-20 py-20 px-4 relative bg-[#1c1100]/50  justify-center items-center flex-col">
        {" "}
        <h1 className="text-2xl sm:text-4xl xl:text-5xl font-bold text-white ">
          {catsProds.categories.name}{" "}
          {lang === "en" ? (
        ...
        {catsProds.categories.products.length === 0 ? (
          <h1 className="text-xl sm:text-3xl xl:text-4xl font-bold text-white z-[0]">
