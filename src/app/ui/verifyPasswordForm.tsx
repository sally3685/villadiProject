...
import { VerifyPassAction } from "../actions/auth";
import { redirect, useRouter } from "next/navigation";
import { toast } from "react-toastify";

export default function VerifyPassForm({ t, lang }: { t: any; lang: string }) {
  const [state, action] = useActionState(VerifyPassAction, undefined);
  ...
  useEffect(() => {
    if (state) {
      if (state.success) redirect(`/${lang}/PasswordReset`);
    }
  }, [state]);
...
function SubmitButton({ lang }: { lang: any }) {
  ...
  return (
    <button
      ...
      type="submit"
    >
      {lang === "en" ? "verify code" : "التحقق من الكود"}
    </button>
  );
}
